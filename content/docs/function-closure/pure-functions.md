...
> const a = (p) => p * 1.1;
> const b = (p) => p * (1 + tax);
> const c = (arr, x) => { arr.push(x); return arr; };
> ```

> [!TIP]
> **Đáp án:** chỉ `a` pure. `b` phụ thuộc biến ngoài `tax` (không deterministic). `c` mutate tham số `arr` (side effect ẩn) — viết pure: `[...arr, x]`.

> [!NOTE]
> **Câu 2:** Đoạn này in gì, và vì sao?
> ```js
> const nums = [3, 1, 2];
...
> [!TIP]
> **Đáp án:** `true [1, 2, 3]`. `sort` **mutate tại chỗ** và trả về chính mảng đó → `nums` bị đổi. Muốn pure: `[...nums].sort()`.
...
> [!IMPORTANT]
> 1. Pure = **deterministic** (cùng input → cùng output) **+ không side effect**.
> 2. Side effect: sửa biến ngoài, **mutate tham số**, I/O, DOM, `console.log`, `Date.now`/`Math.random`.
> 3. Mutate object/array truyền vào là **side effect ẩn** (truyền theo tham chiếu) → trả bản mới.
> 4. Mutate: `push/pop/shift/unshift/splice/sort/reverse`. An toàn: `map/filter/slice/concat/[...]`.
> 5. **Referential transparency** → memoize/cache an toàn.
> 6. Kiến trúc: **functional core, imperative shell** — dồn side effect ra rìa.
